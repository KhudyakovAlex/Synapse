...
- [x] Если запрос не поддержан текущим контрактом, сразу честно говорить об этом вместо имитации выполнения
- [x] Если модель пишет "сделано/подтвердите", но не вернула реальный `dbPatch/navigation/action`, рантайм подменяет ответ на честный отказ
...
- [x] Выбирать активную световую сцену для локации и помещения через `SCENE_NUM`
- [x] Настраивать световые сцены светильников через отдельный `action` с созданием или обновлением записей в `LUMINAIRE_SCENES`
- [x] Создавать новые графики через отдельный `action`
- [x] Полностью редактировать существующие графики через отдельный `action` с полной заменой набора точек
- [ ] Удалять графики
- [x] Создавать новые пункты расписания через отдельный `action`
- [x] Редактировать существующие пункты расписания через отдельный `action`
- [x] Удалять пункты расписания через отдельный `action` с подтверждением в UI
