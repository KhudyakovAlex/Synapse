...
- `action`:
  ...
  - если запрос нельзя выразить этим списком действий, `dbPatch` и `navigation`, модель должна сразу честно ответить, что операция пока не поддерживается
  ...
  - удаление графика сейчас не поддерживается отдельным `action`, поэтому такой запрос должен завершаться честным отказом без имитации успеха
...
- Если при этом нет реального `dbPatch/navigation/action`, а текст выглядит как "сделано" или "подтвердите", рантайм подменяет ответ на честный отказ
