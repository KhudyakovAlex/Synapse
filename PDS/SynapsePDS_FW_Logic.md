...
## 5. Сброс контроллера железной кнопкой

### 5.1. Короткое нажатие кнопки

5.1.1. Короткое нажатие сбрасывает только пароль к заводскому.
...
5.1.3. Остальные данные FW-USM не изменяются.

### 5.2. Долгое нажатие кнопки

5.2.1. Долгое нажатие сбрасывает все настройки прошивки (не исполняемый код) в заводское состояние.
...
5.2.3. После сброса контроллер требует инициализации линии DALI.

## 6. Индикация на контроллере

### 6.1. Нижний светодиод

6.1.1. Нижний светодиод мигает, если идёт работа с линией DALI:
- Отправка команд устройствам;
- Чтение данных с устройств;
- Инициализация линии.

6.1.2. В остальное время светодиод не горит.

### 6.2. Лесенка светодиодов

6.2.1. Лесенка светодиодов загорается вся, если есть хотя бы одно Bluetooth-соединение с телефоном.

6.2.2. Если нет подключённых телефонов — вся лесенка не горит.

6.2.3. Количество горящих светодиодов не отражает количество подключённых телефонов (только факт наличия хотя бы одного).

## 7. Детализация логики

Подробное описание логики работы прошивки разделено на следующие документы:

- **SynapsePDS_FW_Logic_Phone** — раздел 1 «Взаимодействие с телефоном»
- **SynapsePDS_FW_Logic_PNR** — раздел 1 «Пуско-наладочные работы»
- **SynapsePDS_FW_Logic_Settings** — раздел 1 «Настройка»
- **SynapsePDS_FW_Logic_Control** — раздел 1 «Оперативное управление»
- **SynapsePDS_FW_Logic_Sensors** — раздел 1 «Автоматическая работа по датчикам»
- **SynapsePDS_FW_Logic_Schedule** — раздел 1 «Работа по расписанию»
