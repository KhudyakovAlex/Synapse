...
- ICO_NUM — номер иконки контроллера (целое)
- INTERFACE_DATA — интерфейсные данные (энное количество байтов)
...
3.3. Передача данных контроллера от приложения контроллеру:
`[FW.CONTR_SET(...)]`

3.4. Передача данных контроллера от контроллера приложению:
`[USM.CONTR_SET(...)]`

3.5. Запрос от приложения контроллеру:
`[FW.CONTR_GET()]` — запрос данных контроллера.
В ответ на эту телегу прошивка должна ответить телегой `[USM.CONTR_SET(...)]`.
...
sequenceDiagram
    ...
    Приложение->>Прошивка:[FW.CONTR_GET()]
    Прошивка->>Приложение:[USM.CONTR_SET(...)]
```
