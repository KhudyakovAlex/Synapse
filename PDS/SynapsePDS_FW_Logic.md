...
7.5.6. Прошивка:
- Проверяет, что в локации нет устройств (или переносит их в корень если устройства в локации есть);
- Обновляет FW-USM;
- Рассылает изменения телефонам.
...
7.7.2.3. Прошивка:
- Обновляет соответствующие поля в FW-USM;
...
7.8.4. **Связь понятий**:
- Поддействие всегда привязано к действию через **SUBACTIONS.ACTION_ID**;
- Действие может входить в набор действий через **ACTIONS.ACTION_SET_NUM** (если -1, то не входит);
- Действие имеет позицию в наборе через **ACTIONS.POS** для определения порядка перебора.
