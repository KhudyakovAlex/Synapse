...
`[FW.IDATA_SET(XXX)]` — где XXX — блок интерфейсных данных в base64.

3.3. Передача от контроллера приложению:
`[USM.IDATA_SET(XXX)]` — где XXX — блок интерфейсных данных в base64.

3.4. Запрос от приложения контроллеру:
`[FW.IDATA_GET()]` — запрос блока интерфейсных данных.
В ответ на эту телегу прошивка должна ответить своей телегой `[USM.IDATA_SET(XXX)]`.
...
sequenceDiagram
    ...
    Приложение->>Прошивка:[FW.IDATA_GET()]
    Прошивка->>Приложение:[USM.IDATA_SET(XXX)]
```
