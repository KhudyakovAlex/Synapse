...
**Последнее изменение:** 11.12.2025, 11:51 МСК
...
- После изменения имени/иконки: `[ICO]Name` (ICO — номер иконки в квадратных скобках).
...
5.2.4. При отправке телеги телефону прошивка должна получить подтверждение. При отсутствии подтверждения — повторная отправка.

5.2.5. Телеги от прошивки отправляются широковещательно ВСЕМ подключённым телефонам.

### 5.3. Обновление даты и времени

5.3.1. При каждом подключении телефона к контроллеру приложение отправляет текущие дату и время.

5.3.2. Прошивка обновляет свои внутренние часы для работы расписания.

## 6. Загрузка USM контроллера в телефон

### 6.1. Первое подключение к контроллеру

6.1.1. При первом подключении телефона к контроллеру приложение запрашивает пароль.

6.1.2. После успешной аутентификации прошивка отправляет полный дамп FW-USM телефону.

6.1.3. Дамп включает:
- Рабочие данные (все таблицы FW-USM);
- Интерфейсные данные (блок IDATA).

6.1.4. Приложение создаёт запись в таблице **CONTROLLERS** своей APP-USM.
