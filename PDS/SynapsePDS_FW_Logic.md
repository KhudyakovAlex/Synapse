...
**Последнее изменение:** 11.12.2025, 21:23 МСК
...
### 7.5. Создание, изменение и удаление локаций, распределение по ним устройств
...
### 7.6. Создание, изменение и удаление групп, распределение по ним устройств
...
### 7.7. Именование и раздача имён и пиктограмм контроллеру, локациям, устройствам
...
- Включение сцены у светильника/группы/локации/объекта;
...
### 7.9. Создание действий и добавление поддействий

7.9.1. **Создание действия**: приложение отправляет телеграмму с изменением:
...
7.9.2. **Добавление поддействия**: приложение отправляет телеграмму с заполнением записи **SUBACTIONS[j]**:
- **ACTION_ID** = `i`;
...
7.9.4. Никаких DALI-команд не отправляется, т.к. действие ещё не выполняется, а только настраивается.

### 7.10. Назначение действий на присутствие и отсутствие датчиков присутствия

7.10.1. Приложение отправляет телеграмму с изменением:
...
7.10.3. Датчик начинает отрабатывать действия при соответствующих событиях (если АВТО включено).
