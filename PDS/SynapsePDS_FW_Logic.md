...
**Последнее изменение:** 12.12.2025, 11:43 МСК
...
6.1.1. При первом подключении телефона к контроллеру прошивка сразу отправляет полный дамп FW-USM.

6.1.2. Дамп включает:
- Рабочие данные (все таблицы FW-USM);
- Интерфейсные данные (блок IDATA).

6.1.3. Приложение создаёт запись в таблице **CONTROLLERS** своей APP-USM и распаковывает полученные данные.

6.1.4. Приложение проверяет пароль и при неверном пароле блокирует работу с контроллером.

### 6.2. Повторное подключение

6.2.1. Прошивка отправляет полный дамп FW-USM для синхронизации с APP-USM телефона.

6.2.2. Приложение обновляет свою APP-USM данными из FW-USM.
...
6.3.2. Приложение делает 3 попытки восстановления связи с интервалом 1 секунда между попытками.

6.3.3. После неудачных попыток приложение выкидывает пользователя в список контроллеров.

6.3.4. Прошивка отправляет актуальное состояние FW-USM для синхронизации.

### 6.4. Одновременная работа нескольких телефонов

6.4.1. Все телеграммы от контроллера отправляются одновременно всем подключённым телефонам.

6.4.2. При подключении второго/третьего/четвёртого телефона прошивка отправляет им текущий полный дамп FW-USM.

6.4.3. Все изменения FW-USM, вызванные действиями любого из телефонов, рассылаются всем подключённым телефонам.

6.4.4. Каждое приложение обновляет свой APP-USM при получении телеграмм от прошивки.

## 7. Пуско-наладочные работы

### 7.1. Сохранение IDATA

7.1.1. Блок интерфейсных данных IDATA хранится в энергонезависимой памяти контроллера. Он хранит все редко изменяемые ПНР-данные кроме ПНР-данных в таблице CONTROLLERS.

7.1.2. При любом изменении интерфейсных данных через приложение прошивка:
- Получает телеграмму с новым блоком IDATA;
- Записывает его в энергонезависимую память.

7.1.3. При инициализации линии DALI блок IDATA сбрасывается.
...
- Добавляет светильники в FW-USM с:
  ...
  - Начальными значениями VAL_BRIGHT, VAL_TW и т.д. из устройства-свтильника DALI;
  - `STATUS = A`.
- Включает светильник (DALI-команда) после присвоения адреса.

7.2.4. Прошивка инициализирует сцены 0-4 во всех DALI-светильниках и своей USM:
...
### 7.5. Локации и распределение по ним устройств

7.5.1. **Создание локации**: приложение отправляет телеграмму с изменением **LOCATIONS[i].EXIST** = `T` для свободного слота. при этом в телеге возможны и другие изменения в том числе в idata  где хранится имя локации

7.5.3. **Перемещение устройства в локацию**: приложение отправляет телеграмму с изменением **LUMINAIRES[j].LOCATION_ID** = `i`.
аналогично для кнопок и датчиков

7.5.5. **Удаление локации**: приложение отправляет телеграмму с изменением **LOCATIONS[i].EXIST** = `F`.
- Если в локации есть устройства, всем им назначается **LOCATIONS_ID = -1**;

### 7.6. Группы и распределение по ним устройств

7.6.1. **Создание группы**: приложение отправляет телеграмму с изменением **GROUPS[i].EXIST** = `T` и **GROUPS[i].LOCATION_ID** = `loc` (или -1 для корня). при этом в телеге возможны и другие изменения в том числе в idata  где хранится имя группы

7.6.3. **Добавление светильника в группу**: приложение отправляет телеграмму с изменением **LUMINAIRES[j].GROUP_ID** = `i`.
- Прошивка отправляет DALI-команду присвоения светильника группе i;

7.6.5. **Удаление группы**: приложение отправляет телеграмму с изменением **GROUPS[i].EXIST** = `F`.
- Прошивка убирает у всех светильников с **GROUP_ID** = `i` привязку к группе (GROUP_ID = -1);
- Отправляет соответствующие DALI-команды;

### 7.7. Именование и назначение пиктограмм контроллеру, локациям, устройствам

#### 7.7.1. Изменение имени и иконки локаций и устройств

7.7.1.1. Изменение имени/иконки локациям и устройствам — это изменение интерфейсных данных в блоке IDATA.

7.7.1.2. Приложение отправляет телеграмму с новым блоком IDATA.

#### 7.7.2. Изменение имени, иконки и пароля контроллера

7.7.2.1. Изменение имени, иконки и пароля контроллера — это изменение полей **CONTROLLERS.NAME**, **CONTROLLERS.ICO_NUM**, **CONTROLLERS.PASSWORD** (отдельные поля, не в IDATA).

7.7.2.2. Приложение отправляет телеграмму с изменёнными полями контроллера.

7.7.2.4. Если изменилось имя или иконка контроллера:
- Прошивка обновляет имя Bluetooth-устройства;
- Новое имя формата: `[ICO]Name`.

### 7.8. Действия, поддействия и наборы действий

Немного теории.

7.8.1. **Поддействие (таблица SUBACTIONS)** — элементарное изменение системы освещения:
- Включение сцены у светильника/группы/локации/контроллера;
- Установка температуры белого света (TW);
- Переключение режима АВТО.
и т.п.
Со стороны пользователя поддействаия создаются внутри действий и при удалении последних вместе с ними же и удаляются.

7.8.2. **Действие (таблица ACTIONS)** — состоит из одного или нескольких поддействий, выполняемых одним пакетом. Например, действие "Вечерний свет" может включать:
- Включение сцены 2 в гостиной;
- Включение сцены 1 в коридоре;
- Установка температуры 2700K во всех светильниках.
Навешиваются на кнопки, датчики, события расписания.
Не существует какого-то спрвочника действий: со сороны пользователя действия создаются каждый раз как обработчики нажатий кнопок, сигналов датчиков, события в расписании.

7.8.3. **Набор действий (отдельной таблицы нет)** — группа действий, объединённых для перебора. Используется для привязки к кнопке нескольких действий, которые переключаются последовательно при каждом КОРОТКОМ нажатии на кнопку. Например:
- Действие 1: "Яркий свет" (позиция 1);
- Действие 2: "Средний свет" (позиция 2);
- Действие 3: "Тусклый свет" (позиция 3).
Навешиваются ТОЛЬКО на короткие нажатия кнопок. Специальной таблицы в БД с наборами нет. В поле **BUTTONS.ACTION_SET_SHORT_NUM**кнопки устанвливается число, аналогичное число записываем в поля **ACTIONS.ACTION_SET_NUM** одного или нескольких действий. Вот и вся связь.

### 7.9. Добавление/удвление поддействия в/из действие

7.9.1. опишем процесс добавления поддействия в действие здесь и сразу т.к. он позже будет встечаться несколько раз

7.9.2. Добавление поддействия
- Телега с записью **SUBACTIONS[j]**:
  - **ACTION_ID** = `i`;
...
7.9.3. Удаление поддействия
- **SUBACTIONS[j].ACTION_ID** = -1

### 7.10. Назначение действий на датчики присутствия

7.10.1. Назначение на присутствие
- **PRES_SENSORS[i].ACTION_OCCUPANCY_ID** = `j` (j - ID действия из числа свободных);

7.10.1. Снятие назначения на присутствие
- **PRES_SENSORS[i].ACTION_OCCUPANCY_ID** = -1
- Прошивка всем поддействиям с ACTION_ID = очищаемому ACTION_OCCUPANCY_ID ставит ACTION_ID = -1

7.10.2. Назначение на отсутствие
- **PRES_SENSORS[i].ACTION_VACANCY_ID** = `j` (j - ID действия из числа свободных).

7.10.1. Снятие назначения на отсутствие
- **PRES_SENSORS[i].ACTION_VACANCY_ID** = -1
- Прошивка всем поддействиям с ACTION_ID = очищаемому ACTION_OCCUPANCY_ID ставит ACTION_ID = -1

7.10.3. После этого пользователь добавляет в действие поддействия (см. 7.9. Добавление поддействий в действия), но это уже отдельные телеги.

### 7.11. Привязка датчиков освещённости к группам светильников

7.11.1. Приложение отправляет телеграмму с изменением **BRIGHT_SENSORS[i].GROUP_ID** = `j`.

7.11.2. Прошивка последовательно переводит светильники группы в сцены 0-4 (DALI-команды).

7.11.3. Для каждой сцены N (0-4) прошивка:
...
### 7.12. Назначение действий на настенные кнопки

7.12.1. Приложение отправляет телеграмму с изменением:
...
7.12.2. Кнопка начинает отрабатывать действия при нажатиях.
...
8.1.3. Прошивка отправляет DALI-команду установки параметра устройству.
...
8.3.2. **Активация расписания**: приложение отправляет телеграмму с изменением **CONTROLLERS.IS_SCHEDULE** = `T`.

8.3.3. Прошивка начинает проверку событий расписания каждую минуту.

8.3.4. **Деактивация расписания**: приложение отправляет **CONTROLLERS.IS_SCHEDULE** = `F`.
...
9.1.2. Прошивка отправляет DALI-команду установки параметров светильнику.
...
9.3.3. Если АВТО выключается — датчики перестают влиять на освещение. Если АВТО включается — датчики начинают отрабатывать действия.
