...
- **TIMESTAMP** (uint32_t) — текущие дата и время в формате Unix timestamp (количество секунд с 01.01.1970 00:00:00 UTC); обновляется при каждом подключении телефона.
...
- **MIN_LEVEL** (uint8_t) — минимальный уровень яркости светильника;
- **MAX_LEVEL** (uint8_t) — максимальный уровень яркости светильника;
- **FADE_TIME** (uint8_t) — время плавного перехода между уровнями яркости.
...
- **HOLD_TIME** (uint8_t) — задержка перед срабатыванием действия на отсутствие.
- **STATUS** (char[1]) — текущее состояние устройства: A (ACTIVE) — штатная работа, C (CHANGE) — замена устройства, F (FAILURE) — неполадка.
...
- **BUTTON_PANEL_ID** (short) — кнопочная панель, в которой кнопка расположена;
...
- **BUTTON_SHORT_ID** (short) — кнопка, на короткое нажатие которой навешено действие; -1 — действие не привязано к короткому нажатию кнопки (может быть привязано к датчику или событию расписания);
- **POS** (uint8_t) — позиция действия в наборе (начиная с 1); для перебора нескольких действий одной кнопкой.
