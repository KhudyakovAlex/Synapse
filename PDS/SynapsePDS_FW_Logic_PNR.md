...
**Последнее изменение:** 12.12.2025, 14:57 МСК

## 1. Сохранение IDATA

1.1. Блок интерфейсных данных IDATA хранится в энергонезависимой памяти контроллера. Он хранит все редко изменяемые ПНР-данные кроме ПНР-данных в таблице CONTROLLERS.
...
5.2. **Перемещение устройства в локацию**: приложение отправляет телеграмму с изменением **LUMINAIRES[j].LOCATION_ID** = `i`. Аналогично для кнопок и датчиков.
...
## 7. Именование и назначение пиктограмм контроллеру, локациям, устройствам

### 7.1. Изменение имени и иконки локаций и устройств

7.1.1. Изменение имени/иконки локациям и устройствам — это изменение интерфейсных данных в блоке IDATA.

7.1.2. Приложение отправляет телеграмму с новым блоком IDATA.

### 7.2. Изменение имени, иконки и пароля контроллера
...
12.1.1. Добавление первого действия в набор:
- **BUTTONS[i].ACTION_SET_SHORT_NUM** = `set_num`;
- **ACTIONS[j].ACTION_SET_NUM** = `set_num`, **ACTIONS[j].POS** = 1.

12.1.2. Добавление второго и последующих действий:
- **ACTIONS[k].ACTION_SET_NUM** = `set_num`, **ACTIONS[k].POS** = `pos`.

12.1.3. После добавления действия пользователь добавляет в действие поддействия (см. раздел 9), но это уже отдельные телеги.

12.1.4. Удаление действия из набора:
- **ACTIONS[j].ACTION_SET_NUM** = -1;
- Прошивка проверяет, остались ли ещё в наборе действия, и если нет, делает: **BUTTONS[i].ACTION_SET_SHORT_NUM** = -1.

### 12.2. Долгое нажатие (одно действие)

12.2.1. Добавление действия на долгое нажатие:
- **BUTTONS[i].ACTION_LONG_ID** = `j`.

12.2.2. Удаление действия:
- **BUTTONS[i].ACTION_LONG_ID** = -1.
