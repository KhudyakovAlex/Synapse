...
**Последнее изменение:** 13.12.2025, 11:30 МСК
...
4.2. Замена стартуется телеграммой с изменением **LUMINAIRES[i].STATUS** = `C` (или **PRES_SENSORS[i].STATUS** = `C` и т.д.).
...
4.5. Если устройство не найдено, у него ставится статус `F`.

4.6. Прошивка рассылает изменения всем телефонам.
...
6.1. **Создание локации**: приложение отправляет телеграмму с изменением **LOCATIONS[i].EXIST** = `T` для свободного слота. При этом в телеге возможны и другие изменения, в том числе в IDATA, где хранится имя локации.
...
6.3. **Удаление локации**: приложение отправляет телеграмму с изменением **LOCATIONS[i].EXIST** = `F`. Если в локации есть устройства, всем им назначается **LOCATIONS_ID = -1**.

## 7. Группы и распределение по ним устройств

7.1. **Создание группы**: приложение отправляет телеграмму с изменением **GROUPS[i].EXIST** = `T` и **GROUPS[i].LOCATION_ID** = `loc` (или -1 для корня). При этом в телеге возможны и другие изменения, в том числе в IDATA, где хранится имя группы.
...
7.3. **Удаление группы**: приложение отправляет телеграмму с изменением **GROUPS[i].EXIST** = `F`. Прошивка убирает у всех светильников с **GROUP_ID** = `i` привязку к группе (GROUP_ID = -1) и отправляет соответствующие DALI-команды.
