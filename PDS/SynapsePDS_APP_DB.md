...
- **SCENE_NUM** (INTEGER) — световая сцена, включённая для всего объекта (0,1,2,3,4)
- **TIMESTAMP** (INTEGER) — текущие дата и время в формате Unix timestamp (количество секунд с 01.01.1970 00:00:00 UTC)
...
- **MIN_LEVEL** (INTEGER) — минимальный уровень яркости светильника;
- **MAX_LEVEL** (INTEGER) — максимальный уровень яркости светильника;
- **FADE_TIME** (INTEGER) — время плавного перехода между уровнями яркости.
- **STATUS** (TEXT) — текущее состояние устройства: A (ACTIVE) — штатная работа, C (CHANGE) — замена устройства, F (FAILURE) — неполадка.
...
- **HOLD_TIME** (INTEGER) — задержка перед срабатыванием действия на отсутствие.
- **STATUS** (TEXT) — текущее состояние устройства: A (ACTIVE) — штатная работа, C (CHANGE) — замена устройства, F (FAILURE) — неполадка.
...
- **SCENE_BRIGHTNESS_0** (INTEGER) — целевая освещённость для сцены 0;
- **SCENE_BRIGHTNESS_1** (INTEGER) — целевая освещённость для сцены 1;
- **SCENE_BRIGHTNESS_2** (INTEGER) — целевая освещённость для сцены 2;
- **SCENE_BRIGHTNESS_3** (INTEGER) — целевая освещённость для сцены 3;
- **SCENE_BRIGHTNESS_4** (INTEGER) — целевая освещённость для сцены 4;
- **STATUS** (TEXT) — текущее состояние устройства: A (ACTIVE) — штатная работа, C (CHANGE) — замена устройства, F (FAILURE) — неполадка.
...
- ID (INTEGER, PK) — идентификатор в соответствующем массиве в прошивке;
- **BUTTON_SHORT_ID** (INTEGER) — кнопка, на короткое нажатие которой навешено действие; -1 — действие не привязано к короткому нажатию кнопки (может быть привязано к датчику или событию расписания);
- **POS** (INTEGER) — позиция действия в наборе (начиная с 1); для перебора нескольких действий одной кнопкой.
...
- **ACTION_ID** (INTEGER) — действие; -1 — не назначено.
