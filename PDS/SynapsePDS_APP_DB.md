...
2.2. Большинство таблиц имеет два параллельных типа идентификаторов (поскольку в прошивке ID нет):
...
- **IS_AUTO** (INTEGER) — T/F — главный выключатель автоматического режима для всего контроллера;
...
- LOCATION_ID (INTEGER, FK, NULL) — локация; NULL, если светильник без локации (в корне);
- GROUP_ID (INTEGER, FK) — группа, в которую включён светильник; если не в группе — NULL;
- **DALI_ADDR** (INTEGER) — короткий адрес DALI;
- **LOCATION_NUM** (INTEGER) — идентификатор локации для привязки светильника к ней в прошивке; 0 — светильник без локации;
...
- LOCATION_ID (INTEGER, FK, NULL) — идентификатор локации; NULL, если группа без локации (в корне);
- **LOCATION_NUM** (INTEGER) — идентификатор локации для привязки группы к ней в прошивке; 0 — группа без локации;
...
- LOCATION_ID (INTEGER, FK, NULL) — локация; NULL, если датчик без локации (в корне);
...
- LOCATION_ID (INTEGER, FK, NULL) — локация; NULL, если панель без локации (в корне);
