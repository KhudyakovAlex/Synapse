...
1.1. **USM** (Unit System Model) — виртуальная модель системы освещения, включающая как данные устройств DALI-линии, так и метаданные (названия локаций, светильников и т.п.), так и данные для автоматизации (расписание и .т.п), так и много еще чего.
...
Телеги в АПК имеют хожедение в двух форматах:
- **JSON** - где в виде текотового JSONа передаются данные из таблиц APP-USM, в том виде, в каком они в этих таблицах лежат и в каком во всём цивилизованном мире их передавать принято. В этом JSONе в виде нормальных значений находятся в том числе ID и интерфейстные данные из таблиц. В отлицие от их вида в телегах USML, где они подаются одним бинарным блоком обозванным IDATA.
- **USML** - бинарный формат с хитрожопым форматом, но зато размером в разы меньшим, чем JSON-вид.
...
- **Данные** - собственно данные в виде последовательностей байтов идущих одна за другой без разделителей в том виде, в каком они храняться в переменных языка Си
...
3.2.1.3. Формироване идет строго в порядке, определенном в SynapsePDS_FW_DB.md - ## 5. Сруктура БД прошивки (USM прошивки).
