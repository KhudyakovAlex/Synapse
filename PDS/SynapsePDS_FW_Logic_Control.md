...
1.3. Если светильник находится в локации с включённым АВТО, оперативное управление временно переводит **LOCATIONS[loc].IS_AUTO** = `F`.
...
3.1. **У локации**: приложение отправляет телеграмму с изменением **LOCATIONS[i].IS_AUTO** = `T` или `F`.

3.2. **У контроллера**: приложение отправляет телеграмму с изменением **CONTROLLERS.IS_AUTO** = `T` или `F`.
