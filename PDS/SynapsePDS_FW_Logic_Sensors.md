...
Описать логику автоматической работы прошивки по датчикам присутствия и освещённости.
...
## 3. Работа по датчикам

### 3.1. Датчики присутствия

3.1.1. Прошивка постоянно опрашивает датчики присутствия в линии DALI (в периоды затишья).

3.1.2. При регистрации события присутствия на датчике i:
...
3.1.3. При регистрации события отсутствия на датчике i:
- Аналогичные проверки флагов АВТО;
- Прошивка выполняет действие **PRES_SENSORS[i].ACTION_VACANCY_ID**.

3.1.4. Действие отсутствия срабатывает через задержку **PRES_SENSORS[i].DELAY** (HOLD_TIME).

3.1.5. Если есть желание деактивировать датчик — у него убираются действия (ACTION_OCCUPANCY_ID = -1, ACTION_VACANCY_ID = -1).

### 3.2. Датчики освещённости

3.2.1. Прошивка постоянно опрашивает датчики освещённости в периоды затишья.

3.2.2. Если у датчика i назначена группа (**BRIGHT_SENSORS[i].GROUP_ID** != -1):
- Прошивка проверяет флаги АВТО аналогично п. 3.1.2;
...
3.2.3. Подкрутка яркости работает только если у группы включена одна из сцен 0-4.

3.2.4. Любое изменение яркости группы, отличное от включения сцены, отключает подкрутку до следующего включения сцены.

3.2.5. Если есть желание деактивировать датчик — у него убирается привязка к группе (GROUP_ID = -1).

### 3.3. Логика режима АВТО

3.3.1. **Главный флаг АВТО** (**CONTROLLERS.IS_AUTO**):
- Если `F` — все датчики игнорируются;
- Если `T` — работают флаги АВТО локаций.

3.3.2. **Флаг АВТО локации** (**LOCATIONS[i].IS_AUTO**):
- Если `F` — датчики этой локации игнорируются;
- Если `T` — датчики локации отрабатывают действия.

3.3.3. **Устройства в корне** (без локации):
- Работают при **CONTROLLERS.IS_AUTO** = `T`.

3.3.4. **Временное отключение АВТО при оперативном управлении**:
