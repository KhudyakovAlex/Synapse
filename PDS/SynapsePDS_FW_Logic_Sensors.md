...
1.1. Прошивка постоянно опрашивает датчики присутствия в линии DALI (в периоды затишья).
...
1.4. Действие отсутствия срабатывает через задержку **PRES_SENSORS[i].DELAY** (HOLD_TIME).

1.5. Если есть желание деактивировать датчик — у него убираются действия (ACTION_OCCUPANCY_ID = -1, ACTION_VACANCY_ID = -1).
...
2.3. Подкрутка яркости работает только если у группы включена одна из сцен 0-4.
