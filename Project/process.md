...
flowchart LR
    ...
    DSUI( СпецРазр<br>UI )
    DSAPP( СпецРазр<br>Приложение )
    DSDB( СпецРазр<br>База данных )
    DSP(СпецРазр<br>Протокол обмена<br>Приложение-Прошивка)
    DSFW( СпецРазр<br>Прошивка )

    UIUXD((UX-дизайн<br>Худяков))
    UIUX( UX-макет )
    APPD(( Прогр.<br>Булатов ))
