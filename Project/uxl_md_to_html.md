...
**ВАЖНО (1:1 / Full Screen):** UXL-движок открывает прототип в новой вкладке и подключает `./uxl.css` и `./uxl.js`
относительно папки текущего HTML-документа. Поэтому конвертер обязан **дополнительно** копировать свежие ассеты рядом с
каждым HTML-документом, содержащим UXL:
- `INDEX/PDS/uxl.css` и `INDEX/PDS/uxl.js` (для страниц `INDEX/PDS/*.html`)
- `INDEX/PRD/uxl.css` и `INDEX/PRD/uxl.js` (если там появятся UXL-блоки)
