...
5.1. Контроллер хранит константы:
- серийник: XXXXXXXX (8 цифр);
- название в Bluetooth: SYNAPSE XXXXXXXX.

5.2. Контроллер хранит переменные (меняются через приложение):
- пароль: YYYY (4 цифры);
...
12.2. Если есть желание деактивировать конкретный датчик присутствия — убираем у него действия.

12.3. Действие всех датчиков на отдельно взятую локацию может быть активировано/деактивировано тумблером АВТО у этой локации в приложении или настенной кнопкой с соответствующим ACTION Локация.Авто = OFF.

12.4. Если группа или отдельный светильник претерпевают оперативное управление, они перестают принимать действия от датчиков до момента включения у них сцены.

## 13. Датчики освещенности

13.1. К датчику может быть привязана одна или несколько групп из той же локации, что и датчик, которыми он управляет для поддержания искомой освещённости.

13.2. К группе можно привязать больше одного датчика. В этом случае яркость группы определяет датчик, которому больше других темно, несмотря на пересветы в зонах других датчиков.

13.3. Если есть желание деактивировать конкретный датчик освещённости — убираем у него привязку к группе.

13.4. Действие всех датчиков на отдельно взятую локацию может быть активировано/деактивировано тумблером АВТО у этой локации в приложении или настенной кнопкой с соответствующим ACTION Локация.Авто = OFF.
...
13.6. Если при сохранении сцены (задания целевой освещённости) светильник полностью выключен, он в подкрутке освещённости по датчику участвовать не будет.
...
16.2. Данные в контроллере делятся на:
...
16.3. Программная кнопка на контроллере, сбрасывающая:
- **долгим нажатием** настройки прошивки (не исполняемый код) в заводское состояние;
- **коротким нажатием** только пароль в заводской.

16.4. Автоматическая работа по датчикам и расписанию при соответствующих флагах, выставленных в локациях.

16.5. Опрос текущего состояния устройств в периоды затишья.

16.6. При отправке устройствам команд на изменение состояния последующий опрос того, подействовали ли эти команды, не предполагается. Выстрелили сообразно стандартам DALI и забыли. Сказали светильнику включиться на 33, считаем, что он включился на 33.

## 17. Прошивка. ПНР

17.1. Инициализация линии DALI со сбросом настроек устройств к заводским по команде от приложения.

17.2. При инициализации кроме DALI-настроек удаляются все локации (и создаётся одна, в которую все вновь найденные устройства складируются), устройства, группы, расписание.

17.3. Передача в приложение текущего статуса инициализации устройств для информирования пользователя о ходе процесса.

17.4. Перед инициализацией все устройства широковещательно выключаются (у кнопок и датчиков выключаются светодиоды). После успешной выдачи устройству короткого адреса DALI оно включается.

17.5. Добавление новых устройств в линию без пересбора линии.

17.6. Замена вышедших из строя устройств без пересбора линии:
...
18.3. Только вертикальная ориентация.

18.4. Приложение используется в первую очередь для ПНР и настройки. Задача оперативного управления лежит на настенных кнопках.

18.5. Структура локальной базы данных в приложении аналогична БД в контроллере.

18.6. В БД приложения хранятся данные всех актуальных контроллеров, к которым подключалось приложение.

18.7. При подключении к контроллеру приложение актуализирует соответствующие записи в своей БД.

18.8. Заход в контроллер возможен только при наличии Bluetooth-соединения с контроллером.

18.9. При пропадании связи с контроллером и после нескольких безуспешных попыток её восстановить приложение выкидывает пользователя в список контроллеров.

18.10. При появлении связи с контроллером, из которого вылетели по причине пропадания связи, заходим в него автоматом в прежнюю локацию (если не успели зайти в другой контроллер).

18.11. Перекодировки из человеческой шкалы 0..100 в нечеловеческую 0..255, равно как и из HSB в RGB, производятся в приложении в момент сохранения значений в базу. В базе значения RGB(W) разложены в R, G и B (W).
...
19.7. BRIGHT_SENSORS — датчики освещённости
...
20.10. **Настройки датчика освещённости** — активация/деактивация, целевая освещённость, управляемая группа светильников, название, DALI-параметры, удаление датчика.
