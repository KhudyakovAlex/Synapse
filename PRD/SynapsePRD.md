...
3.2. **Устройство** — DALI-устройство в линии: светильник, датчик, кнопочная панель.
...
3.9. **Оперативное управление** — ручная установка пользователем яркости, цвета и/или температуры света светильников; управление ржимом АВТО в локациях.
...
3.12. **Режим АВТО** - изменение освещения в локации на основе сигналов от датчиков. Включается/выключается тумблером на странице локации в приложении.
...
12.3. Действие на отсутствие срабатывает, когда у датчик регистрирует отсутствие присутствия.
...
13.4. Действие всех датчиков на отдельно взятую локацию может быть активировано/деактивировано тумблером АВТО у этой локации в приложении или настенной кнопкой с соответствующим ACTION Локация.Авто = OFF или через расписание.
...
13.6. Если при сохранении сцены (и, соответственно, задания целевой освещённости) светильник полностью выключен, он в подкрутке освещённости по датчику участвовать не будет.

13.7. Любое изменение яркости светильников в группе отличное от включения одной из ее 5 сцен отключает подкрутку яркости в ней по датчику освещенности.

13.8. Включение у группы любым способом одной из ее сцен переводит группу в режим поддержания целевой освещенности.

## 14. Действия

14.1. Действие (ACTION) — набор из манипуляций:
- включение сцены у отдельного светильника, группы, локации иди всего объекта;
...
14.3. Действие может распространяться на любые локации и весь объект назвисимо от расположения инициирующего устройства.

## 15. Расписание

15.1. Расписание задается в виде набора событий (EVENTS), при возникновении каждого из которых выполняется действие (ACTION).

15.2. События могут быть ежедневными или назначаться на оперделенные дни недели.

15.2. Расписание одно для всего контроллера.
...
15.5. Если на одно время назначено и ежедневное событие и событие на день недели, сначала выполняется первое, затем второе.

15.6. На одно и то же время нельзя назначить для ежедневных события и/или два события по дням недели.
...
17.2. Данные в контроллере делятся на:
...
17.3. Программная кнопка на контроллере, сбрасывающая:
- **долгим нажатием** настройки прошивки (не исполняемый код) в заводское состояние;
- **коротким нажатием** только пароль в заводской.

17.4. Автоматическая работа по датчикам и расписанию при соответствующих флагах, выставленных в локациях.

17.5. Опрос текущего состояния устройств в периоды затишья.

17.6. При отправке устройствам команд на изменение состояния последующий опрос того, подействовали ли эти команды, не предполагается. Выстрелили сообразно стандартам DALI и забыли. Сказали светильнику включиться на 33, считаем, что он включился на 33.

## 18. Прошивка. ПНР

18.1. Инициализация линии DALI со сбросом настроек устройств к заводским по команде от приложения.
...
## 21. Приложение. Основные страницы

21.1. **Настройки приложения** — переключение тем интерфейса, невключённые разрешения Андроида (с переходом на их включение).

21.2. **Список контроллеров** — старые и вновь найденные контроллеры, выбор контроллера для подключения.

21.3. **Контроллер** — список локаций контроллера, оперативное управление всеми светильниками, расписание.

21.4. **Настройки контроллера** — название, иконка, пароль, инициализация/расширение линии, обновление прошивки контроллера, удаление контроллера.

21.5. **Локация** — оперативное управление светильниками локации, переход на страницы светильников, датчиков и кнопок локации, расписание, АВТО/НЕАВТО.

21.6. **Настройки локации** — название, иконка, перемещение иконок светильников, формирование групп светильников, перемещение устройств между локациями.

21.7. **Светильник** — расширенное оперативное управление.

21.8. **Настройки светильника** — группа, название, иконка, DALI-параметры, удаление светильника.

21.9. **Группа светильников** — оперативное управление группой, сохранение сцен для группы.

21.10. **Настройки группы светильников** — название, сгруппированные светильники, удаление группы.

21.11. **Настройки датчика присутствия** — задержка (HOLD_TIME), назначение действий при срабатывании, название, DALI-параметры, удаление датчика.

21.12. **Настройки датчика освещённости** — управляемая группа светильников, название, DALI-параметры, удаление датчика.

21.13. **Настройки кнопочной панели** — назначение действий на кнопки, название, удаление панели.

21.14. **Расписание** — редактирование расписания.

## 22. Приложение. Подключение к контроллеру

22.1. При заходе в приложение — список контроллеров:
...
22.2. При подключении к новому контроллеру запрашивается пароль.

22.3. Заводской пароль указывается на упаковке контроллера или на его корпусе.

22.4. Зашедший в контроллер пользователь может поменять пароль.

22.5. Сброс к заводским настройкам сбрасывает пароль на заводской.

22.6. При следующих подключениях к контроллеру пароль не запрашивается.

22.7. Контроллер можно удалить из списка. При следующем поиске он появится среди вновь найденных.

22.8. Контроллеру можно назначить другое имя и пиктограмму. Имя и пиктограмма (номер) хранятся в контроллере.

22.9. Если при предыдущем закрытии приложения пользователь находился в контроллере в какой-то локации и при запуске приложения этот контроллер доступен, приложение автоматически заходит в этот контроллер и в эту локацию.

## 23. Приложение. ПНР

23.1. Находясь в контроллере, пользователь может запустить процесс инициализации линии DALI.

23.2. После отправки команды инициализации на контроллер показывается прогресс инициализации, любые действия с контроллером до её окончания невозможны, пользователь может выйти из контроллера или закрыть приложение (инициализацию проводит контроллер автономно).

23.3. Возможно расширение линии за счет вновь подключенных устройств. Устройства добавляются с дефолтными параметрами в новую локацию.

23.4. Если устройство вышло из строя (что должно быть показано на иконке устройства), пользователь может, зайдя в настройки устройства, осуществить привязку нового экземпляра на старое место (если он по типу совпадает).

23.5. Установка ПНР-параметров DALI-устройств на соответствующих страницах настроек.

## 24. Приложение. LLM

24.1. Общение с LLM идет в текстовом или голосовом формате (в обе стороны).

24.2. При каждом запросе вместе с сообщением пользователя LLM передаются:

- Системный промпт с уставом свето-нейро-службы.

- JSON с текущим состоянием системы освещения данного контроллера.

24.3. LLM выполняет следующие функции:
...
24.4. Использование LLM должно полностью покрывать функционал кнопочного интерфейса и ускорять процесс настройки за счет групповых и повторяющихся действий.
...
Кнопка7.Короткое — Свет11.Сцена4 (бра 2)  
Кнопка8.Короткое — Свет11.Сцена0 (бра 2)  
Кнопка7.Длинное — Свет11.Сцена4 (бра 2)  
Кнопка8.Длинное — Свет11.Сцена0 (бра 2)  